...
- **彩色边框** = 该设备类型启用此插件
- **透明边框 + 半透明** = 该设备类型禁用此插件
- 主开关联动：开启插件时清空所有设备禁用，关闭插件时标记全部设备类型禁用
- 持久化策略：全部启用 → 持久化启用；全部禁用 → 持久化禁用；部分禁用 → 持久化禁用 + 临时启用

设备类型通过 `navigator.userAgentData` + `navigator.userAgent` + `navigator.maxTouchPoints` 综合识别。
